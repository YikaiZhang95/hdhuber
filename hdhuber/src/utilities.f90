...
SUBROUTINE objfun_huber (intcpt, bb, ka, y, lam, nobs, delta, objval) !change
  IMPLICIT NONE
  INTEGER :: nobs, j
  DOUBLE PRECISION :: intcpt, bb, ka (nobs), y (nobs), lam, objval
  DOUBLE PRECISION :: fh (nobs), xi (nobs), xi_tmp, delta
  xi = 0.0D0
  !check loss
  DO j = 1, nobs
    fh(j) = ka(j) + intcpt
    xi_tmp = y(j) - fh(j)
    IF (Abs(xi_tmp) <= delta) THEN
      xi_tmp = xi_tmp**2/2.0D0
    ELSE 
      xi_tmp = (Abs(xi_tmp) - 0.50D0 * delta) * delta
    END IF
    xi(j) = xi_tmp
  ENDDO
  objval = (lam/2.0D0) * bb + Sum(xi)/(nobs) 
END SUBROUTINE objfun_huber

! This code is partially modified from fmim.c from in the source code of R
SUBROUTINE opt_int_huber (lmin, lmax, nobs, ka, bb, y, lam, delta, objval, lhat)
  IMPLICIT NONE
  INTEGER :: nobs
  DOUBLE PRECISION :: lmin, lmax, lhat, objval, ka (nobs), bb, y (nobs), lam, delta
  DOUBLE PRECISION :: a, b, d, e, p, q, r, u, v, w, x
  DOUBLE PRECISION :: t2, fu, fv, fw, fx, xm, tol, tol1, tol3
  REAL(KIND = SELECTED_REAL_KIND(10, 99)) :: eps
  DOUBLE PRECISION, PARAMETER :: gold = (3.0D0 - Sqrt(5.0D0)) * 0.5D0
  ...
  e = 0.0D0
  objval = 0.0D0
  CALL objfun_huber(x, bb, ka, y, lam, nobs, delta, objval)
  fx = objval
  fv = fx
  fw = fx
  tol3 = tol / 3.0D0
  ...
  main_loop: DO
    ...
    objval = 0.0D0
    CALL objfun_huber(u, bb, ka, y, lam, nobs, delta, objval)
    fu = objval
